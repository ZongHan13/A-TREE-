...
class ATree() {
    ...
    def setRecoveryToStringWithAnd(set: Set[String]): String = {
     ...
     def selectAn_S_that_maximizes_insect_in_Hen(target_set: Set[Set[String]], _expression:String): Set[String] = {
        ...
        val hen_no_self_and_NoOrnode = hen_no_self.filter(!_._2.expression.contains('∨'))
         hen_no_self_and_NoOrnode.foreach(x => println(x._2.expression + "!!!!!!!!---------------------------------"))
        for (target <- target_set) {
            for ((id,node) <- hen_no_self_and_NoOrnode) {
