...
class ATree() {
  ...
  private def reorganize(_expression: String): List[String] = {
    // println("here")
    var u =
      ListBuffer(List(_expression)).map(_.flatMap(_.split("[\\^∨]")).toSet).toSet
      //ListBuffer(List(_expression)).map(_.flatMap(_.split('^')).toSet).toSet
  ...
  def exprToPredicateSet(expr: String): Set[String] = {
    ...
    println("StringSets : " + stringSet.mkString(", "))
    stringSet
      ...
      while (j < childNodes(i).parent.size - 1) { ////// 這裡應該要從parent 先去掉 newNode自己
        ...
        if (
          parentExpressionContainsNewExpression(
            childNodes(i).parent(j).expression,
            newNode.expression
          )
        ) {
          // println("current node:" + newNode.expression)
          ...
          // println("childNode:" + childNode.expression)
          childNodes(i).parent(j).childs -= childNodes(i)
          if (!childNodes(i).parent(j).childs.contains(newNode)) {
            childNodes(i).parent(j).childs += newNode
          }
          childNodes(i).parent -= childNodes(i).parent(j)
          // childNode.parent += node
          // node.childs += childNode
...
//println(tree.hen(5872187552040000L))
...
// println("---------------- result ----------------")
println(tree.hen(5872749877997477l).expression)
println(tree.hen(5872749877997477l).childs.size)
//println(tree.hen(5872749877997477l).childExprs.size ) 
println(tree.hen(5872749877997477l).childs(0).expression)
println(tree.hen(5872749877997477l).childs(1).expression)
println(tree.hen(176333).expression)
println(tree.hen(176333).parent.size)
println(tree.hen(176333).parent(0).expression)
println(tree.hen(176333).childs.size)
