...
class ATree() {
    ...
    def insert(expression: String): Unit = {   //插入新的expression
        if (root_Node_map.getOrElse(expression, 0) == 0) {
            ...
            connect_leaf_to_secondlayer(   split_to_leaf(expression), new_node  )
    ...
    private def connect_leaf_to_secondlayer(leaf_predicate: Array[String], _rootNode: root_Node ): Unit = { //把leaf node連接到second layer node
        for (i <- leaf_predicate) {
            ...
            // }
            if (_rootNode.expression.contains(i)) {
                if(_rootNode.child.contains(leaf_Node_map(i))) {
                        
                } else {
                    _rootNode.child += leaf_Node_map(i)
                }
            }
            ...
            for (second_layer_node <- second_layer_node_List) {
                if (second_layer_node.expression.contains(leaf_count_kv_pair(i)._1) & second_layer_node.expression.contains(leaf_count_kv_pair(i + 1)._1)) {
                    ...
                    for(second_layer_node_child <- second_layer_node.child) {
                        if (second_layer_node_child.expression.contains(leaf_count_kv_pair(i)._1)){
                    ...
                    for (delete_node <- for_delete_node) {
                        second_layer_node.child -= delete_node
                    }
                ...
                // -----------tttttttttttttttttttttttte
                var for_test_enough_or_not = 0
                var predicates_will_be_add_back = ListBuffer[String]()
                for (child <- second_layer_node.child) {
